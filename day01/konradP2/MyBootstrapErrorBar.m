...
for i = 1:1000
    inds = 1+floor(rand(size(angle))*length(angle));
     mySpikes = spikes(nNeuron, inds);
     myAngles = angle (inds);
    for j = 1:length(angles)-1
        angleIndex = find(myAngles>=angles(j) & myAngles<angles(j+1));
%         spikeIndex = find(
        bsSpikes(i,j) = mean(mySpikes(angleIndex));
    end
end
%another hint. Use matlab sort function
bsSpikes = sort(bsSpikes);
M = mean(bsSpikes);
L = M - bsSpikes(26,:);
U = bsSpikes(975,:) - M;
errorbar(angles(1:end-1)+pi/16, M, U, L);
