...
plot(dot_ax, nanrunmean(m_mr1c',1),'LineWidth',2)
avgact = mean(nanmean((m_mr1c(:,dot_ax>=200)+m_mr2c(:,dot_ax>=200))/2,2));
mirroredact = m_mr2c;
mirroredact(:,dot_ax >= 200) = 2*avgact - mirroredact(:,dot_ax >= 200);
plot(dot_ax, nanrunmean(mirroredact',1),'--','LineWidth',2)
plot([0 0],[20 70],'k-');
plot([200 200],[20 70],'k-');
plot([390 390],[20 70],'k-');
xlabel('Time (ms)')
ylabel('Firing rate (sp/s) (flipped)')

figure('Color', 'white');  hold on;
urgency = (m_mr1c+m_mr2c)/2;
urgency = urgency - mean(urgency(:,dot_ax == 200));
%plot(dot_ax, nanrunmean(urgency',1), '-', 'LineWidth', 2);
plot(dot_ax, urgency', '-', 'LineWidth', 2);
plot(xlim, [0 0], 'k--');
xlabel('Time (ms)')
ylabel('Urgency signal (sp/s)')
